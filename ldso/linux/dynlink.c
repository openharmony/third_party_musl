...
static inline __attribute__((always_inline)) Sym *lookup_sym_in_dso(struct dso *dso, struct verinfo *verinfo,
	struct sym_info_pair s_info_g, struct sym_info_pair s_info_s, uint32_t gh, uint32_t gho, size_t ghm)
{
	uint32_t *ght = dso->ghashtab;
	Sym *sym = NULL;
	if (ght) {
		if (gnu_hash_filter(ght, ghm, gho, gh)) {
			sym = gnu_lookup(s_info_g, ght, dso, verinfo);
	...
	if (__predict_false(dso->adlt)) {
		...
		if (sym && !is_adlt_dso_sym(dso, sym - dso->syms)) {
			sym = NULL;
		}
		if (!sym) {
...
static inline struct symdef find_sym2(struct dso *dso, struct verinfo *verinfo, int need_def, int use_deps, ns_t *ns)
{
	...
	struct sym_info_pair s_info_s = {0, 0};
	uint32_t gh = s_info_g.sym_h, gho = gh / (8 * sizeof(size_t));
	size_t ghm = 1ul << gh % (8*sizeof(size_t));
	struct symdef def = {0};
	...
	for (; dso; dso=use_deps ? *deps++ : dso->syms_next) {
		...
		Sym *sym = lookup_sym_in_dso(dso, verinfo, s_info_g, s_info_s, gh, gho, ghm);
		if (!sym) continue;
