...
​      These features are designed specifically for when musl_extended_function is true. These interfaces also need to be called on platforms other than `__LITEOSA__`. These interfaces can only be used when the flag for controlling thread affinity series interfaces at runtime is turned on.
