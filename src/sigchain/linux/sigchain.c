...
static void signal_chain_handler(int signo, siginfo_t* siginfo, void* ucontext_raw)
{
    ...
    if ((sa_flags & SA_SIGINFO)) {
        SIGCHAIN_PRINT_INFO("%{public}s call usr sigaction for signal: %{public}d", __func__, signo);
        ...
        if (sig_chains[signo - 1].sig_action.sa_handler == SIG_IGN) {
            ...
            SIGCHAIN_PRINT_INFO("%{public}s call usr sa_handler: %{public}p for signal: %{public}d", __func__, signo);
