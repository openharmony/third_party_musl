...
/* Build the printf_info passed to the handler. */
...
/* Handle a registered (user-defined) conversion specifier.
 * Must be called with *s pointing at the specifier character, in the
 * real (non-pre-pass) round.
...
static int printf_registered(FILE *f, const char **s, va_list *ap,
                              unsigned fl, int w, int p,
                              unsigned int user_mod, unsigned ps)
{
                       ...
                       union arg *nl_arg, int *nl_type, char nl_arg_filled, int flags)
{
	...
	for (;;) {
		...
		if (__printf_modifier_table) {
			/* Match a registered multi-byte modifier and record its bit;
			 * parsing then continues with the conversion specifier. */
			__handle_registered_modifier_mb((const char **)&s, &user_mod);
		}
#endif
		...
		if (!st) {
			...
			if (f && __printf_function_table) {
				...
				unsigned char ch = (unsigned char)*s;
				if (ch <= UCHAR_MAX && __printf_function_table[ch]) {
					int ret = printf_registered(f, &s, ap, fl, w, p,
					                            user_mod, ps);
